...
| Arrays & Hashing | **Longest Consecutive Sequence** | $O(n)$ | $O(n)$ | Hashset (map[int]struct{}), then iterate from head to store curr max, update glob max. |
...
| Graphs | **Course Schedule II** | $O(m+n)$ | $O(m+n)$ | create adj list and indeg list. |
